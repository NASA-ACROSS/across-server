...
from across_server.core.schemas.bandpass import BandpassSchema


class TestBandpass:
    def test_converter_wavelength(self) -> None:
        schema = BandpassSchema(
    ...
    def test_converter_energy(self) -> None:
        schema = BandpassSchema(min=2.0, max=4.0, unit=tools_enums.EnergyUnit.keV)
        result = schema.bandpass_converter()
        assert "central_wavelength" in result
        assert "bandwidth" in result

    def test_converter_frequency(self) -> None:
        schema = BandpassSchema(min=3e14, max=6e14, unit=tools_enums.FrequencyUnit.Hz)
        result = schema.bandpass_converter()
        assert "central_wavelength" in result
        assert "bandwidth" in result

    def test_converter_missing_min_max_energy(self) -> None:
        schema = BandpassSchema(unit=tools_enums.EnergyUnit.keV)
    ...
    def test_converter_missing_min_max_frequency(self) -> None:
        schema = BandpassSchema(unit=tools_enums.FrequencyUnit.Hz)
        with pytest.raises(UnproccessableEntityException):
            schema.bandpass_converter()

    def test_converter_missing_wavelength(self) -> None:
        schema = BandpassSchema(unit=tools_enums.WavelengthUnit.ANGSTROM)
        with pytest.raises(UnproccessableEntityException):
            schema.bandpass_converter()
