...
from across_server.routes.tle.schemas import TLE


class TestTLESchemas:
    def test_epoch_calculation(self, mock_tle_data, todays_epoch):
    ...
    def test_epoch_calculation_old_year(self, mock_tle_data_explorer_6):
        tle = TLE(**mock_tle_data_explorer_6)
        assert tle.epoch.year == 1959

    def test_epoch_invalid_format(self, mock_tle_data_invalid_epoch):
